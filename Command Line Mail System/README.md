...
* **State Machines:** Because `select()` multiplexes I/O, the server cannot halt execution to wait for a specific client's input. Instead, each connected client is assigned a `Client` struct containing a state variable. This allows the server to remember exactly where each client is in the protocol sequence and handle multiple connections concurrently.
...
Both SMTP2 and SMP were strictly implemented to handle edge cases properly:
* **Byte-Stuffing (Dot-Stuffing):** The client prepends an extra `.` if a user's body line starts with a dot. The server strips this leading dot before saving to disk, preventing premature body termination. The server reapplies it when sending file contents during the `READ` command.
* **Out-of-Sequence Commands:** If a client sends a command in the wrong sequence, the server state machine catches it and returns `ERR Bad sequence`.
...
* **Max Clients:** The server restricts concurrent connections to a maximum of 50 (`MAX_CONCURRENT_CLIENTS = 50`). Additional connection attempts are immediately closed.
...
* **Client/Server Disconnection:** When recv returns a negative number, the client prints server disconnected and exits immediately.
...
* `userfile.txt`: Sample text file containing registered users and passwords.
